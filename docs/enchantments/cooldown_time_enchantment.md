...
- **与[涡旋](./spiral.md)对比**：涡旋同样给牌加重放 1，但**不加消耗**——冷却时间可视为"涡旋+消耗"，多一次触发但用完即弃。
- **与[华彩](./glam.md)对比**：华彩首打自带重放 1、用完即禁用（本场仅一次双倍），冷却时间首打即带重放且打出后牌直接消耗离场——两者都是一次性双倍，冷却时间不限"第一次"，配合重放类效果更灵活。
