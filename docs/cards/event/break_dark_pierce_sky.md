...
- **曝是附带收割手段，不依赖唱词数量**：只要打出时**有唱词**（哪怕只剩 1 句），就给随机 <span style="color:#3aa675;font-weight:600">1</span> 个敌人施加 1 层[曝](/powers/expose_power.md)：该敌人的[幻扰](/powers/illusion_disturb_power.md)施加异常状态成功率降半、**死亡后直接消逝**（不触发复活和亡语——分裂、复活类被动全部跳过）、消逝时把自身所有负面状态传染给全部存活队友。对会分裂/复活的敌人（如[史莱姆国王](/monsters/normal/slime_king_monster.md)的无尽幻象）是精准克制。随机目标走战斗目标随机数，多人模式两端一致。
