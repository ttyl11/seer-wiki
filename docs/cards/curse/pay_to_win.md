...
- **⚠️ 每次进入手牌都扣钱，不是只扣一次**：扣钱判定挂在"进入手牌"这一事件上——抽到扣、被洗牌重抽再扣、任何效果把它塞回手里也扣。放任它在[抽牌堆](/mechanics/draw_pile.md)和[弃牌堆](/mechanics/discard_pile.md)之间循环，每转一圈就再失去一次金币（向下取整）。越早处理掉，总损失越小。
