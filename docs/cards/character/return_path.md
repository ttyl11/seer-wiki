...
- **伤害基于全场格挡+属性总和——雪球型全体攻击**：伤害 = (己方+敌方所有格挡值 + 己方+敌方所有[全属性](/mechanics/all_attributes.md)层数绝对值) × 伤害百分比。场上格挡和属性越多，伤害越高——是对格挡流/力量流敌人的反制核弹。
- **双刃剑——移除自身格挡和属性换伤害**：移除的是"所有生物"的格挡和属性，包括你自己。所以你会失去自己的格挡、[力量](/powers/strength_power.md)、[命中](/powers/accuracy_power.md)、[速度](/powers/speed_power.md)、[防御](/powers/defense_power.md)。**别在自己叠了大量格挡/属性时打**，否则自毁增益——这张牌适合在"自己没什么属性积累，敌人叠了一堆"时反制。
