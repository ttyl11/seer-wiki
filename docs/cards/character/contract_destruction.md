...
- **超过阈值再翻倍**：若消耗的 PP 总和超过阈值（20/15），伤害**再 ×2（即 4 倍）**，同时获得 1 层[缓冲](/powers/buffer_power.md)。举例：手牌有 3 张 PP 卡，PP 分别为 7+7+7=21，超过阈值 20 → 伤害 = 21×2×2 = 84 点。升级后阈值降至 15，同样 21 点 PP 触发翻倍 → 84 点。
- **PP 清零是永久代价**：被消耗的 PP 卡在消耗前 PP 被设为 0。即使后续通过[逍遥游](./carefree_journey.md)回收，该牌**会恢复 PP**（逍遥游回收时恢复 PP），所以配合逍遥游可以回收高 PP 卡。但若不回收，被消耗的牌就彻底报废了。所以这张牌是"all-in"式的爆发——用 PP 换一次性巨额伤害。
