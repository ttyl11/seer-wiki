...
- **残局打出惩罚更小**：生物数按打出瞬间的场面实时计算——敌人死亡后立即移出计数。残局只剩 <span style="color:#3aa675;font-weight:600">1</span> 敌 + 自己时打出，速度只 <span style="color:#3aa675;font-weight:600">-2</span>（少抽 <span style="color:#3aa675;font-weight:600">1</span> 张），却照常拿 <span style="color:#3aa675;font-weight:600">2</span> 抽 <span style="color:#3aa675;font-weight:600">2</span> 能量收尾。先减员再打这张牌是标准操作顺序。
- **负速度是减益，能被清除**：负速度在游戏中是减益类型，可被清除减益的效果直接清掉。清掉后等于白嫖了一轮爆发——抽牌和能量到手，速度惩罚被抹除。所以配合减益清除能力可以把这张牌变成无损爆发。
- **重复打出惩罚累加**：速度是计数型能力，多次打出负层数直接叠加——4 生物场面连打 <span style="color:#3aa675;font-weight:600">2</span> 次就是速度 <span style="color:#3aa675;font-weight:600">-8</span>，之后每回合少抽 <span style="color:#3aa675;font-weight:600">4</span> 张。第二次打出前先想清楚战斗还能拖几回合。
