...
- **转态无缝衔接**：百天浮世络到期消失的同一瞬间（对方回合结束时），遗物立刻施加下一个状态——你的下个回合直接享受怀生菩怜态/拒赦怒威态效果，中间没有"裸奔回合"，也不会打断你的出牌节奏。
- **怀生菩怜态是控制态**：高血量（>54%）时进入，变光系，打出牌后随机对敌人施加 1 种异常状态。适合安全时打控制，配合多打牌可频繁上异常。
- **拒赦怒威态是输出态**：低血量（≤54%）时进入，变混沌系，**每回合开始**[先制](/powers/first_strike_power.md)+1（9 回合 = 9 张减费牌），攻击伤害 +30%。适合残血时速杀——被打残了就自动转输出模式。
...
- **每次状态转换抽弃回**：进入或退出任一三态时抽 1 张牌、可弃 0~1 张牌（可取消）、回 1 点生命。完整流转一次（进入百天→退出百天→进入怀生→退出怀生）就有 4 次触发 = 4 张牌 + 4 点生命；中途被切到拒赦再算 2 次。两类转换的**时机不同**：百天→怀生/拒赦发生在**对方回合结束**（百天衰减消失的瞬间无缝衔接，抽到的牌下回合直接可用）；怀生→拒赦发生在**受击掉血到 54% 以下的瞬间**（敌人打你的当场就切换，连 DoT 掉血、自伤都算）。
