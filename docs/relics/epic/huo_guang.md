...
- **必定暴击是核弹级**：战斗开始自动获得北冥之刃，所有攻击必定暴击。暴击结算为基础暴击倍率 ×1.5 与刃的加成**连乘**：之刃时期一张 10 点伤害的攻击牌实际打 10×1.5×1.5=**22.5** 点——是描述暗示的 1.95 倍还要更高的实际收益。无需堆暴击率，拿到这张遗物后每张攻击牌都是暴击。
- **5场战斗后质变升级**：每打赢一场战斗计数+1（遗物图标上的计数器实时显示），达到 5 场后北冥之刃升级为北冥狂刃。狂刃时期暴击总倍率达到 1.5×2=**3 倍**，且每次暴击伤害结算后对目标追加等量固定伤害——打 30 点暴击伤害就再追加 30 点固定伤害，输出近乎再翻一倍。越往后打越强，是典型的滚雪球遗物。
- **狂刃固伤的三条隐藏规则**：①追加量按**总伤害**计算——包含被敌方格挡挡掉的部分，面对高格挡敌人时固伤反而更痛（等于变相穿透格挡）；②多段攻击每一跳独立结算、独立追加，5 连击就是 5 次固伤；③固伤延迟到**目标回合开始时**引爆（不可格挡、不吃攻击增伤），可被免疫固定伤害的效果挡掉——面对固伤免疫的敌人时狂刃的追加失效，但必暴+100% 暴击伤害依然生效。
...
- **性价比**：史诗稀有度偏保守——必暴本身就是机制级强度（等效于暴击率 100%），实际数值还比描述更高（+50%/+100%），狂刃成型后配合多段攻击牌单回合能打出夸张的爆发。攻击流牌组拿到即质变。
