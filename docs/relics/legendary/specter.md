...
- **虚无触发**：仅响应敌人造成的攻击伤害（非攻击伤害不触发），且**被格挡的也算**——只要那次攻击的总伤害大于 0 就叠 1 层缓冲，不用实际掉血。
- **充能球池**：原版 4 种（闪电/冰霜/黑暗/等离子）+ 9 种 mod 自定义充能球（圣杯、PP、Multi、火山、海洋、森林、塔、星辰、神谕），共 13 种随机（不含玻璃球）。
- **能量伤害**：已损失生命 × 10%，对随机敌人造成非攻击伤害——**可被格挡**（不是固定伤害，不吃固伤免疫/修改）。
...
- **虚无：挨打叠缓冲**：受到敌人攻击伤害后获得1层[缓冲](/powers/buffer_power.md)（下次HP损失归零并消耗1层）。关键细节：**被格挡的攻击也算**——敌人打你 20 点被格挡挡满，照样白拿 1 层缓冲。配合高格挡build等于"每次被攻击都在攒免伤"，面对高频攻击的敌人可以叠出多层缓冲形成无敌墙。注意只响应攻击伤害，异常/固伤等非攻击伤害不触发。
- **元素+能量：免费充能球+残血反伤**：回合开始免费生成1个充能球（13种池子随机，含mod球），回合结束对随机敌人造成已损失生命10%的非攻击伤害（可被格挡）。越残血伤害越高——血量越低输出越猛，配合虚无的缓冲可以安全保持低血线打输出。
- **生命：回合结束续航**：回合结束回复3点生命+为1张随机PP牌回复1点PP。每回合稳定续航，配合能量的残血反伤形成"低血高输出→回合结束回血"的循环。PP回复随机选牌，PP牌多的牌组更容易回关键牌。
- **永恒与能量的血线矛盾**：永恒需要生命严格大于所有敌人才给先制+2，能量需要已损失生命才有伤害——满血时永恒触发但能量伤害为零，残血时能量伤害高但永恒失效。需要权衡血线：开局满血抢先制压制，中后期残血打反伤，靠生命的回血在两者间切换。
