...
- **⚠️ 本地化与源码不一致（实际更强）**：本地化写"下次攻击必定暴击"，实际获得的是[回合必定暴击](/powers/turn_guaranteed_crit_power.md)——**本回合所有攻击**必定暴击（1.5 倍伤害），持续到本回合结束。效果以实际为准。
...
- **⚠️ 实际效果比描述强：整回合必暴，不只是下一击**：本地化写"下次攻击必定暴击"，但实际给的是[回合必定暴击](/powers/turn_guaranteed_crit_power.md)——本回合**所有攻击**都按 1.5 倍结算。满足条件的回合里多段攻击、连打的每一击全部吃满暴击倍率，比"下一击必暴"值钱得多。
